...
class Models_management extends Admin_Controller
{
    ...
    |
    */
    protected $model_name = "model_models_management";
    protected $model_countries = "model_country";

    public $image_up_path = MODELS_IMAGE_UP_PATH;
    public $thumb_up_path = MODELS_THUMB_UP_PATH;
    public $image_show_path = MODELS_IMAGE_PATH;
    public $thumb_show_path = MODELS_THUMB_PATH;
    ...
    */
    public function add()
    {
        ...
        if (isset($_POST) && count($_POST) != 0)
        {
          ...
          $model_type    = $this->input->post("model_type", TRUE);
          ...
          $image1_tmp_name    = $_FILES["userfile"]["tmp_name"];
            ...
            if($this->model_models->hasUsername($username))
            {
                $error = TRUE;
                $data["Error"] = "Y";
            ...
            if(!$error)
            {
                if ($this->modelNameAlias->isExist($is_exist_array) == FALSE)
                {
                    ...
                    if ($uploadstatus != "Error")
                    {
                        ...
                        $uniqTokn = $this->model_models->genUniqToken($username);
                        $hashKey = $this->model_models->genhashKey($cpwd, $uniqTokn);
                        ...
                        $data_array     = array(
                            ...
                            "model_type" => mysql_real_escape_string($model_type),
                        ...
                        $this->model_models->removeLangByModel($last_insert_id);

                        foreach ($language as $key => $value) {
                            $this->model_models->setLanguage($last_insert_id,$value);
                        }
                        ...
                        for($i=0; $i<$cpt; $i++)
                        {
                            ...
                            if($_FILES['image']['tmp_name'][$i] != "")
                            {
                                ...
                                if (move_uploaded_file($_FILES['image']['tmp_name'][$i], MODELS_IMAGE_UP_PATH.$Image1Name)) {
                                    ...
                                    $obj_img->Cropping   = 'Y';
                                    $obj_img->PathImgOld = MODELS_IMAGE_UP_PATH."/".$Image1Name;
                                    $obj_img->PathImgNew = MODELS_THUMB_UP_PATH."/".$Image1Name;
                                    $obj_img->create_thumbnail_images();

                                    $save_gallery = array(
                                        'model_fk' => $last_insert_id,
                                        'image' => $Image1Name,
                                        'added_date' => date('Y-m-d h:i:s A')
                                    );

                                    $this->model_models->save_photos($save_gallery);
    ...
    */
    public function edit()
    {
        ...
        $data['countries'] = $this->modelCountries->get_all_country();
        $data['languages'] = $this->modelLangAlias->getAllLanguages();
        $data['mdlLngs'] = $this->model_models->getLanguageByModel($record_id);
        ...
        if ($this->input->post("delete_gallery_file")=="Y")
        {
            ...
            if (!empty($file_name))
            {
                ...
                $this->model_models->deleteModelPhoto($record_id,$file_name);
                $data["MSG"] = "Selected File has been deleted successfully.";
            }
        ...
        if ($this->input->post("btnsubmit") == "Save & Close")
        {
            ...
            if($this->input->post("btnsubmit")=="Save & Close" && !$error)
            {
                ...
                if(count($language) > 0)
                {
                    $this->model_models->removeLangByModel($record_id);

                    foreach ($language as $key => $value) {
                        $this->model_models->setLanguage($record_id,$value);
                    }
                }
        ...
        $data["model_type"]          = stripslashes($data_record->model_type);
        ...
        $data["gender"]        = stripslashes($data_record->gender);
        ...
        $data['gallery'] = $this->model_models->getModelPicsByPk($record_id);
    ...
    */
    public function view()
    {
        ...
        $data['languages'] = $this->model_models->getLanguageByModel($record_id);
        //get the image gallery for the model
        $data['gallery'] = $this->model_models->getModelPicsByPk($record_id);
