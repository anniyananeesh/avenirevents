...
class Contents_management extends Admin_Controller
{
    ...
    |
    */
    protected $model_name = "model_contents_management";
    public $image_up_path = CONTENTS_IMAGE_UP_PATH;
    public $thumb_up_path = CONTENTS_THUMB_UP_PATH;
    public $image_show_path = CONTENTS_IMAGE_PATH;
    public $thumb_show_path = CONTENTS_THUMB_PATH;
    ...
    */
    public function index()
    {
        ...
        if (!empty($searchBox))
        {
            $where_array[$this->search_field . " LIKE "] = $searchBox . "%";
        }
        ...
        ---------------------------*/
        $where_array["is_active"] = "Y";
        $data_list                = $this->modelNameAlias->get_all_records($where_array, $limit, $offset, $sortWith, $by);
    ...
    */
    public function pages()
    {
        ...
        $data["folder_name"] = $folder;
        $data["module_name"] = $this->getModuleInfo()->module_name;
        $data["page"]        = $folder . "/pages";

        $parent_id           = $this->uri->segment(3);
        $parent_name         = $this->uri->segment(4);
        $data["parent_id"]   = $parent_id;
        $data["parent_name"] = $parent_name;

        $base_url .= HOST_URL . "/" . $folder . "/pages/" . $parent_id . "/" . $parent_name;
        $data_search         = $this->modelNameAlias->auto_complete_data_for_search();
        $data["data_search"] = $data_search;
        ...
        else if ($this->uri->segment(5) == "e")
        {
            $data["MSG"] = "Record has been updated successfully!";
        }
        else if ($this->uri->segment(5) == "d")
        {
            $data["MSG"] = $this->uri->segment(6) . " Records deleted successfully!";
        }
    ...
    */
    public function add()
    {
        ...
        if (isset($_POST) && count($_POST) != 0)
        {
            ...
            $heading_fr = $this->input->post("heading_fr", TRUE);

            $description_sm_en    = $this->input->post("description_sm_en", FALSE);
            $description_sm_ar = $this->input->post("description_sm_ar", FALSE);
            $description_sm_fr = $this->input->post("description_sm_fr", FALSE);

            $description_en = $this->input->post("description_en", FALSE);
            $description_ar = $this->input->post("description_ar", FALSE);
            $description_fr = $this->input->post("description_fr", FALSE);
            ...
            if ($this->modelNameAlias->isExist($is_exist_array) == FALSE)
            {
                ...
                if ($uploadstatus != "Error")
                {
                    ...
                    $data_array     = array(
                        ...
                        "heading_fr" => mysql_real_escape_string($heading_fr),
                        "description_sm_en" => addslashes($description_sm_en),
                        "description_sm_ar" => addslashes($description_sm_ar),
                        "description_sm_fr" => addslashes($description_sm_fr),
                        "description_en" => addslashes($description_en),
                        "description_ar" => addslashes($description_ar),
                        "description_fr" => addslashes($description_fr),
                        "website_title" => addslashes($website_title),
            ...
            if ($data["Error"] == "Y")
            {
                ...
                $data["heading_fr"]        = $heading_fr;
                $data["description_sm_en"]    = $description_sm_en;
                $data["description_sm_ar"] = $description_sm_ar;
                $data["description_sm_fr"] = $description_sm_fr;
                $data["description_en"]    = $description_en;
                $data["description_ar"]    = $description_ar;
                $data["description_fr"]    = $description_fr;
            ...
            {
                redirect(HOST_URL . "/" . $folder . "/pages/" . $parent_id . "/" . $parent_name . "/a");
            }
        }
    ...
    */
    public function edit()
    {
        ...
        $parent_name         = $this->uri->segment(5);
        $data["parent_id"]   = $parent_id;
        $data["parent_name"] = $parent_name;
        ...
        if ($this->input->post("btnsubmit") == "Save & Close")
        {
            ...
            redirect(HOST_URL . "/" . $folder . "/pages/" . $parent_id . "/" . $parent_name . "/e");
        }

        $data["heading_en"]        = stripslashes($data_record->heading_en);
        $data["heading_ar"]        = stripslashes($data_record->heading_ar);
        $data["heading_fr"]        = stripslashes($data_record->heading_fr);
        $data["description_sm_en"]    = stripslashes($data_record->description_sm_en);
        $data["description_sm_ar"] = stripslashes($data_record->description_sm_ar);
        $data["description_sm_fr"] = stripslashes($data_record->description_sm_fr);
        $data["description_en"]    = stripslashes($data_record->description_en);
        $data["description_ar"]    = stripslashes($data_record->description_ar);
        $data["description_fr"]    = stripslashes($data_record->description_fr);
        $data["comments"]          = stripslashes($data_record->comments);
        $data["website_title"]     = stripslashes($data_record->website_title);
        $data["meta_keywords"]     = stripslashes($data_record->meta_keywords);
        $data["meta_description"]  = stripslashes($data_record->meta_description);
    ...
    */
    public function delete()
    {
        ...
        $record_id = $this->uri->segment(3);
        if (!empty($record_id) and $record_id != '-')
        {
        ...
        if ($this->input->post("btnsubmit") == "Delete")
        {
            ...
            redirect(HOST_URL . "/" . $folder . "/pages/" . $parent_id . "/" . $parent_name . "/d/" . count($EditBox));
        }
