...
<div class="page">
	<?php echo form_open(HOST_URL."/".$folder_name."/manage_videos/".$parent_id, 'name="frmListing" id="frmListing"'); ?>
	<div id="module">
		<div class="icon"><img src="<?=ADMIN_IMG_PATH?>/icon_<?php echo $folder_name; ?>.png" alt="<?=$module_name?>" /></div>
		<div><a href="<?=HOST_URL?>/<?=$folder_name?>" class="module_nameBig"><?php echo $module_name; ?> : Videos</a></div>
		<div class="buttons">
			<ul class="actions">
				<a href="<?=HOST_URL?>/<?=$folder_name?>/add_video/<?=$parent_id?>"><li>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td align="left" valign="middle"><table width="100%" border="0" cellspacing="0" cellpadding="0">
						    <tr>
						        <td align="left" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/manage_videos/<?=$parent_id?>/sort/title/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Video URL </a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_title))?$sort_title:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
						        </tr>
					    </table></td>
						<td width="100" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/manage_videos/<?=$parent_id?>/sort/orderby/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Order</a> <img src="<?=ADMIN_IMG_PATH?>/filesave.png" align="absmiddle" border="0" onclick="submitOrderOnly();" style="cursor:pointer;" title="Save Order" /></td>
						<td width="100" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/manage_videos/<?=$parent_id?>/sort/added_date/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Date</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_added_date))?$sort_added_date:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
						<? if (isset($is_featured)){ ?>
						<td width="90" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/manage_videos/<?=$parent_id?>/sort/is_featured/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Featured</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_is_featured))?$sort_is_featured:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
						<? } ?>
						<td width="70" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/manage_videos/<?=$parent_id?>/sort/is_active/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Status</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_is_active))?$sort_is_active:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
						<td width="50" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/manage_videos/<?=$parent_id?>/sort/id/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">ID</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_id))?$sort_id:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
					</tr>
				</table>
			</div>
			<div class="list">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td width="50" align="left" valign="middle">&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?=HOST_URL?>/<?=$folder_name?>/delete_videos/<?=$parent_id?>/<?php echo $value->id; ?>"><img src="<?=ADMIN_IMG_PATH?>/delete.png" alt="Delete" border="0" /></a></td>
						<td align="left" valign="middle"><table width="100%" border="0" cellspacing="0" cellpadding="0">
						    <tr>
						        <td align="left" valign="middle"><a href="<?php echo prep_url($value->video_url); ?>" title="Click to view details" class="listLink" target="_blank"><?php echo $value->video_title; ?></a></td>
						        </tr>
						    </table></td>
...
<script language="javascript">
	...
	function DeleteRecord(){
		document.frmListing.action="<?=HOST_URL?>/<?=$folder_name?>/delete_videos/<?=$parent_id?>";
