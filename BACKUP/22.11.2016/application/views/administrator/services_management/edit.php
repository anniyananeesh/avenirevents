		...
		<div id="panel">
			...
			<div class="rigth-colum">
				<div class="col-contents">
					...
					<!-- ********** IMAGES SECTION ********** -->
					<div id="options-header">
						<div class="options_title">Images (337px X 431px)</div>
					...
					<!-- ********** IMAGES SECTION ********** -->
					<div id="options-header">
						<div class="options_title">Home image (300px X 130px)</div>
						<div class="box_gray_notice">Jpeg, Gif & PNG only</div>
					</div>
					<div class="box">
						<div class="fieldTitle2">Image</div>						
						<div><? echo form_upload('image2','', 'id="image2" class="fileBox"'); ?></div>
						<div class="spacer">&nbsp;</div>
						<? if (!empty($data_record->home_image)){ ?>
						<div class="fieldTitle2">&nbsp;</div>	
						<div class="float1"><img src="<?=$this->image_show_path?><?=$data_record->home_image?>" class="thumb" /> 
						<img src="<?=ADMIN_IMG_PATH?>/cross.png" alt="Delete" hspace="2" border="0" onclick="javascript:if(delConfirmation()==true){DeleteFile('<?=$data_record->home_image?>', 'home_image')}" style="cursor:pointer;" /></div>
						<? } ?>
					</div>
