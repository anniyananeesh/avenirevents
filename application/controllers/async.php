...
class Async extends CI_Controller
{
    ...
    public function signup()
    {
        ...
        //Username checking
        if($this->isAlreadyRegistered(@$username))
        {
            $error = TRUE;
            $data = array(
                'error' => TRUE,
        ...
        if(!$error)
        {
            ...
            $hashKey = $this->model_models->genhashKey(@$pwd, $uniqTokn);

            $save = array(
               TBL_MODELS.'.orderby' => $orderby,
               TBL_MODELS.'.code' => $code,
               TBL_MODELS.'.username' => @$username,
               TBL_MODELS.'.uniq_token' => $uniqTokn,
               TBL_MODELS.'.hash_key' => $hashKey,
               TBL_MODELS.'.r_password' => @$pwd,
               ...
               TBL_MODELS.'.model_type' => @$model_type,
