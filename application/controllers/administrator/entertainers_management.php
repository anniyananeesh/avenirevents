...
class Entertainers_management extends Admin_Controller
{
    ...
    |
    */
    protected $model_name = "model_entertainers_management";
    protected $model_countries = "model_country";

    public $image_up_path = ENTERTAINER_IMAGE_UP_PATH;
    public $thumb_up_path = ENTERTAINER_THUMB_UP_PATH;
    public $image_show_path = ENTERTAINER_IMAGE_PATH;
    public $thumb_show_path = ENTERTAINER_THUMB_PATH;
    public $cv_up_path = ENTERTAINER_CV_UP_PATH;
    ...
    */
    public function __construct()
    {
        ...
        $this->load->model('model_entertainer');
    ...
    */
    public function add()
    {
        ...
        if (isset($_POST) && count($_POST) != 0) {
            ...
            if ($this->model_entertainer->hasUsername(array(
                "email" => $username
            ))) {
                $error         = TRUE;
                $data["Error"] = "Y";
            ...
            if (!$error) {
                ...
                if ($this->modelNameAlias->isExist($is_exist_array) == FALSE) {
                    ...
                    if ($uploadstatus != "Error") {
                        ...
                        $code      = $this->model_entertainer->genUniqCode();
                        ...
                        $this->model_entertainer->removeLangById($last_insert_id);

                        foreach ($language as $key => $value) {
                            $this->model_entertainer->setLanguage($last_insert_id, $value);
                        }
                        ...
                        for ($i = 0; $i < $cpt; $i++) {
                            ...
                            if ($_FILES['image']['tmp_name'][$i] != "") {
                                ...
                                $config                = array();
                                $config['upload_path'] = ENTERTAINER_IMAGE_UP_PATH;
                                if (move_uploaded_file($_FILES['image']['tmp_name'][$i], ENTERTAINER_IMAGE_UP_PATH . $Image1Name)) {
                                    ...
                                    $obj_img->Cropping   = 'Y';
                                    $obj_img->PathImgOld = ENTERTAINER_IMAGE_UP_PATH . "/" . $Image1Name;
                                    $obj_img->PathImgNew = ENTERTAINER_THUMB_UP_PATH . "/" . $Image1Name;
                                    $obj_img->create_thumbnail_images();

                                    $save_gallery = array(
                                        'entertainer_fk' => $last_insert_id,
                                        'image' => $Image1Name,
                                        'added_date' => date('Y-m-d h:i:s A')
                                    );

                                    $this->model_entertainer->savePhotos($save_gallery);
    ...
    */
    public function edit()
    {
        ...
        $data['countries']   = $this->modelCountries->get_all_country();
        $data['languages']   = $this->modelLangAlias->getAllLanguages();
        $data['mdlLngs']     = $this->model_entertainer->getLanguageById($record_id);
        ...
        if ($this->input->post("delete_gallery_file") == "Y") {
            ...
            if (!empty($file_name)) {
                ...
                $this->model_entertainer->deleteModelPhoto($record_id, $file_name);
                $data["MSG"] = "Selected File has been deleted successfully.";
            }
        ...
        if ($this->input->post("btnsubmit") == "Save & Close") {
            ...
            if ($this->input->post("btnsubmit") == "Save & Close" && !$error) {
                ...
                if (count($language) > 0) {
                    ...
                    $this->model_entertainer->removeLangById($record_id);

                    foreach ($language as $key => $value) {
                        $this->model_entertainer->setLanguage($record_id, $value);
                    }
                }
        ...
        $data['gallery'] = $this->model_entertainer->getPicsByPk($record_id);
    ...
    */
    public function view()
    {
        ...
        $data['languages'] = $this->model_entertainer->getLanguageById($record_id);
        //get the image gallery for the model
        $data['gallery']   = $this->model_entertainer->getPicsByPk($record_id);
