...
class Promoter_management extends Admin_Controller
{
    ...
    |
    */
    protected $model_name = "model_promoter_management";
    ...
    public $cv_up_path = COMPANY_CV_UP_PATH;
    ...
    */
    public function __construct()
    {
        ...
        $this->load->model('model_promoter');
    ...
    */
    public function add()
    {
        ...
        if (isset($_POST) && count($_POST) != 0) {
            ...
            if ($this->model_promoter->hasUsername(array(
                "email" => $username
            ))) {
                $error         = TRUE;
                $data["Error"] = "Y";
            ...
            if (!$error) {
                ...
                if ($this->modelNameAlias->isExist($is_exist_array) == FALSE) {
                    ...
                    if ($uploadstatus != "Error") {
                        ...
                        $code      = $this->model_promoter->genUniqCode();
                        ...
                        $this->model_promoter->removeLangById($last_insert_id);

                        foreach ($language as $key => $value) {
                            $this->model_promoter->setLanguage($last_insert_id, $value);
                        }
                        ...
                        for ($i = 0; $i < $cpt; $i++) {
                            ...
                            if ($_FILES['image']['tmp_name'][$i] != "") {
                                ...
                                if (move_uploaded_file($_FILES['image']['tmp_name'][$i], COMPANY_IMAGE_UP_PATH . $Image1Name)) {
                                    ...
                                    $this->model_promoter->savePhotos($save_gallery);
    ...
    */
    public function edit()
    {
        ...
        $data['countries']   = $this->modelCountries->get_all_country();
        $data['languages']   = $this->modelLangAlias->getAllLanguages();
        $data['mdlLngs']     = $this->model_promoter->getLanguageById($record_id);
        ...
        if ($this->input->post("delete_gallery_file") == "Y") {
            ...
            if (!empty($file_name)) {
                ...
                $this->model_promoter->deleteModelPhoto($record_id, $file_name);
                $data["MSG"] = "Selected File has been deleted successfully.";
            }
        ...
        if ($this->input->post("btnsubmit") == "Save & Close") {
            ...
            if ($this->input->post("btnsubmit") == "Save & Close" && !$error) {
                ...
                if (count($language) > 0) {
                    ...
                    $this->model_promoter->removeLangById($record_id);

                    foreach ($language as $key => $value) {
                        $this->model_promoter->setLanguage($record_id, $value);
                    }
                }
        ...
        $data['gallery'] = $this->model_promoter->getCompanyPicsByPk($record_id);
    ...
    */
    public function view()
    {
        ...
        $data['languages'] = $this->model_promoter->getLanguageById($record_id);
        //get the image gallery for the model
        $data['gallery']   = $this->model_promoter->getCompanyPicsByPk($record_id);
