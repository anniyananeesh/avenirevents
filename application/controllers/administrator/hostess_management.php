...
class Hostess_management extends Admin_Controller
{
    ...
    |
    */
    protected $model_name = "model_hostess_management";
    protected $model_countries = "model_country";

    public $image_up_path = HOSTESS_IMAGE_UP_PATH;
    public $thumb_up_path = HOSTESS_THUMB_UP_PATH;
    public $image_show_path = HOSTESS_IMAGE_PATH;
    public $thumb_show_path = HOSTESS_THUMB_PATH;
    public $cv_up_path = HOSTESS_CV_UP_PATH;
    ...
    */
    public function add()
    {
        ...
        if (isset($_POST) && count($_POST) != 0) {
            ...
            $bust       = $this->input->post("bust", TRUE);
            $waist      = $this->input->post("waist", TRUE);
            $hip        = $this->input->post("hip", TRUE);
            $hair_long  = $this->input->post("hair_long", TRUE);
            $eye_color  = $this->input->post("eye_color", TRUE);
            $dress_size = $this->input->post("dress_size", TRUE);
            $shoe_size  = $this->input->post("shoe_size", TRUE);
            $pant_size  = $this->input->post("pant_size", TRUE);

            $model_region          = $this->input->post("model_region", TRUE);
            $model_pref            = $this->input->post("model_pref", TRUE);
            ...
            if ($this->model_hostess->hasUsername(array(
                "email" => $username
            ))) {
                $error         = TRUE;
                $data["Error"] = "Y";
            ...
            if (!$error) {
                ...
                if ($this->modelNameAlias->isExist($is_exist_array) == FALSE) {
                    ...
                    if ($uploadstatus != "Error") {
                        ...
                        $code      = $this->model_hostess->genUniqCode();
                        ...
                        $data_array = array(
                            ...
                            "bust" => mysql_real_escape_string($bust),
                            "waist" => mysql_real_escape_string($waist),
                            "hip" => mysql_real_escape_string($hip),
                            "hair_long" => mysql_real_escape_string($hair_long),
                            "eye_color" => mysql_real_escape_string($eye_color),
                            "dress_size" => mysql_real_escape_string($dress_size),
                            "shoe_size" => mysql_real_escape_string($shoe_size),
                            "pant_size" => mysql_real_escape_string($pant_size),

                            "model_region" => mysql_real_escape_string($model_region),
                            "model_pref" => serialize($model_pref),
                        ...
                        $this->model_hostess->removeLangById($last_insert_id);

                        foreach ($language as $key => $value) {
                            $this->model_hostess->setLanguage($last_insert_id, $value);
                        }
                        ...
                        for ($i = 0; $i < $cpt; $i++) {
                            ...
                            if ($_FILES['image']['tmp_name'][$i] != "") {
                                ...
                                $config                = array();
                                $config['upload_path'] = HOSTESS_IMAGE_UP_PATH;
                                if (move_uploaded_file($_FILES['image']['tmp_name'][$i], HOSTESS_IMAGE_UP_PATH . $Image1Name)) {
                                    ...
                                    $obj_img->Cropping   = 'Y';
                                    $obj_img->PathImgOld = HOSTESS_IMAGE_UP_PATH . "/" . $Image1Name;
                                    $obj_img->PathImgNew = HOSTESS_THUMB_UP_PATH . "/" . $Image1Name;
                                    $obj_img->create_thumbnail_images();

                                    $save_gallery = array(
                                        'hostess_fk' => $last_insert_id,
                                        'image' => $Image1Name,
                                        'added_date' => date('Y-m-d h:i:s A')
                                    );

                                    $this->model_hostess->savePhotos($save_gallery);
    ...
    */
    public function edit()
    {
        ...
        $data['countries']   = $this->modelCountries->get_all_country();
        $data['languages']   = $this->modelLangAlias->getAllLanguages();
        $data['mdlLngs']     = $this->model_hostess->getLanguageById($record_id);
        ...
        if ($this->input->post("delete_gallery_file") == "Y") {
            ...
            if (!empty($file_name)) {
                ...
                $this->model_hostess->deleteModelPhoto($record_id, $file_name);
                $data["MSG"] = "Selected File has been deleted successfully.";
            }
        ...
        if ($this->input->post("btnsubmit") == "Save & Close") {
            ...
            if ($this->input->post("btnsubmit") == "Save & Close" && !$error) {
                ...
                if (count($language) > 0) {
                    ...
                    $this->model_hostess->removeLangById($record_id);

                    foreach ($language as $key => $value) {
                        $this->model_hostess->setLanguage($record_id, $value);
                    }
                }
        ...
        $data["height"]     = stripslashes($data_record->height);
        $data["weight"]     = stripslashes($data_record->weight);
        $data["bust"]       = stripslashes($data_record->bust);
        $data["waist"]      = stripslashes($data_record->waist);
        $data["hip"]        = stripslashes($data_record->hip);
        $data["hair_long"]  = stripslashes($data_record->hair_long);
        $data["eye_color"]  = stripslashes($data_record->eye_color);
        $data["dress_size"] = stripslashes($data_record->dress_size);
        $data["shoe_size"]  = stripslashes($data_record->shoe_size);
        $data["pant_size"]  = stripslashes($data_record->pant_size);

        $data["model_region"]          = stripslashes($data_record->model_region);
        $data["model_pref"]            = unserialize($data_record->model_pref);
        ...
        $data['gallery'] = $this->model_hostess->getPicsByPk($record_id);
    ...
    */
    public function view()
    {
        ...
        $data['languages'] = $this->model_hostess->getLanguageById($record_id);
        //get the image gallery for the model
        $data['gallery']   = $this->model_hostess->getPicsByPk($record_id);
