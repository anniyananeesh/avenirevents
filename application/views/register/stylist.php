...
myApp.controller('SignupCtrl',function($scope, $http, $window) {
	...
	var settings = {
	    url: "<?php echo HOST_URL?>/async/stylist/upldPhotos",
	    dragDrop:false,
	    fileName: "myfile",
	    returnType:"json",
	    showDelete:true,
        ...
        onSuccess:function(files,data,xhr,pd)
		{
		    ...
		    var request = $http({
				method: "post",
				url: "<?php echo HOST_URL?>/async/stylist/setPhoto",
				data: setParams,
	...
	var CVsettings = {
	    url: "<?php echo HOST_URL?>/async/stylist/upldCV",
	    dragDrop:false,
	    fileName: "cvFile",
	    returnType:"json",
	    showDelete:true,
	    ...
	    onSuccess:function(files,data,xhr,pd)
		{
		    ...
		    var request = $http({
				method: "post",
				url: "<?php echo HOST_URL?>/async/stylist/setCV",
				data: setParams,
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			});
      ...
      <?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>',
 		...
 		if(valid)
 		{
 			...
 			if($('#upload-container').children().length == 0)
 			{
 				...
 				var request = $http({
				    method: "post",
				    url: "<?php echo HOST_URL?>/async/stylist/signup",
				    data: params,
				    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
				});
...
<div class="container" ng-controller="SignupCtrl" id="signUpFrmHolder">
                ...
                <div class="col-md-6 <?php echo ($lan == 'ar') ? 'pull-right' : 'pull-left';?>">
                                    ...
                                    <div class="form-group">
                                      ...
                                      <label class="radio-inline">
                                          <input type="radio"  name="model_info" value="hair_stylist" ng-checked="user.model_info == 'hair_stylist'" ng-click="user.model_info = 'hair_stylist'"> Hair Stylist
                                        </label>
                                        <label class="radio-inline">
                                          <input type="radio"  name="model_info" value="makeup_artist" ng-checked="user.model_info == 'makeup_artist'" ng-click="user.model_info = 'makeup_artist'"> Makeup Artist
                                        </label>
                                        <label class="radio-inline">
                                          <input type="radio"  name="model_info" value="hair_make_up" ng-checked="user.model_info == 'hair_make_up'" ng-click="user.model_info = 'hair_make_up'"> Hair & Make Up
                                        </label>
                                         <label class="radio-inline">
                                          <input type="radio"  name="model_info" value="prop_stylist" ng-checked="user.model_info == 'prop_stylist'" ng-click="user.model_info = 'prop_stylist'"> Prop Stylist
                                        </label>
                                        <label class="radio-inline">
                                          <input type="radio"  name="model_info" value="wardrobe" ng-checked="user.model_info == 'wardrobe'" ng-click="user.model_info = 'wardrobe'"> Wardrob
                                        </label>
                                    </div>
                                    <div class="form-group">
                    										<input type="text" class="form-control" placeholder="Not in the list Please Mention here" ng-model="user.model_info" ng-focus="user.model_info = ''">
                    								</div>
  ...
  <div class="border-content-signup" id="successFrmHolder">
    ...
    <div class="clearfix"></div>
    <a href="<?php echo HOST_URL."/";?>" class="btn-footer2 inline-style" style="margin-top: 15px;"><?php echo lang("auto.go_home")?></a>
