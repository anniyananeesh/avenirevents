...
myApp.controller('SignupCtrl',function($scope, $http, $window) {
	...
	var settings = {
	    url: "<?php echo HOST_URL?>/async/hostess/upldPhotos",
	    dragDrop:false,
	    fileName: "myfile",
	    returnType:"json",
	    showDelete:true,
        ...
        onSuccess:function(files,data,xhr,pd)
		{
		    ...
		    var request = $http({
				method: "post",
				url: "<?php echo HOST_URL?>/async/hostess/setPhoto",
				data: setParams,
	...
	var CVsettings = {
	    url: "<?php echo HOST_URL?>/async/hostess/upldCV",
	    dragDrop:false,
	    fileName: "cvFile",
	    returnType:"json",
	    showDelete:true,
	    ...
	    onSuccess:function(files,data,xhr,pd)
		{
		    ...
		    var request = $http({
				method: "post",
				url: "<?php echo HOST_URL?>/async/hostess/setCV",
				data: setParams,
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			});
      ...
      <?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>',
 		...
 		if(valid)
 		{
 			...
 			if($('#upload-container').children().length == 0)
 			{
 				...
 				var request = $http({
				    method: "post",
				    url: "<?php echo HOST_URL?>/async/hostess/signup",
				    data: params,
				    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
				});
