...
<div class="page">
	<?php echo form_open(HOST_URL."/".$folder_name."/pages/".$parent_id."/".$parent_name, 'name="frmListing" id="frmListing"'); ?>
	<div id="module">
		<div class="icon"><img src="<?=ADMIN_IMG_PATH?>/icon_<?php echo $folder_name; ?>.png" alt="<?=$module_name?>" /></div>
		<div><a href="<?=HOST_URL?>/<?=$folder_name?>" class="module_nameBig"><?php echo $module_name; ?> : <?=$parent_name?></a></div>
		<div class="buttons">
			<ul class="actions">
			...
			<div class="list_header">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td width="100" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/pages/<?=$parent_id?>/<?=$parent_name?>/sort/orderby/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Order</a> <img src="<?=ADMIN_IMG_PATH?>/filesave.png" align="absmiddle" border="0" onclick="submitOrderOnly();" style="cursor:pointer;" title="Save Order" /></td>
						<td width="100" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/pages/<?=$parent_id?>/<?=$parent_name?>/sort/added_date/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Date</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_added_date))?$sort_added_date:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
						<? if (isset($is_featured)){ ?>
						<td width="90" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/pages/<?=$parent_id?>/<?=$parent_name?>/sort/is_featured/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Featured</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_is_featured))?$sort_is_featured:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
						<? } ?>
						<td width="70" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/pages/<?=$parent_id?>/<?=$parent_name?>/sort/is_active/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">Status</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_is_active))?$sort_is_active:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
						<td width="50" align="center" valign="middle"><a href="<?=HOST_URL?>/<?=$folder_name?>/pages/<?=$parent_id?>/<?=$parent_name?>/sort/id/by/<?=$sortby?><?=$urlparams?>/offset/<?=$offset?>" class="listHeading">ID</a> <img src="<?=ADMIN_IMG_PATH?>/<?php echo ((isset($sort_id))?$sort_id:"sort_empty.png"); ?>" align="absmiddle" class="sort_image"/></td>
					</tr>
				</table>
			</div>
			<div class="list">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td width="70" align="left" valign="middle">&nbsp;&nbsp;<a href="<?=HOST_URL?>/<?=$folder_name?>/edit/<?php echo $value->id; ?>/<?=$parent_id?>/<?=$parent_name?>"><img src="<?=ADMIN_IMG_PATH?>/edit.png" alt="Edit" border="0" /></a> <?php if ($can_delete=="Y"){ ?><a href="<?=HOST_URL?>/<?=$folder_name?>/delete/<?php echo $value->id; ?>/<?=$parent_id?>/<?=$parent_name?>"><img src="<?=ADMIN_IMG_PATH?>/delete.png" alt="Delete" border="0" /></a><?php } ?></td>
						<td align="left" valign="middle"><table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
